            ...
            with adapter.driver.session() as neo4j_session:                # Consulta mejorada para obtener directamente toda la información de la moto
                ...
                if record:
                    ...
                    moto = {
                        ...
                        "anio": int(record.get('anio', ''))  # Año desde Neo4j
                    ...
                    if adapter.motos_df is not None:
                        ...
                        if not moto_info.empty:
                            ...
                            if 'anio' in moto_row:
                                moto["anio"] = int(moto_row.get('anio'))
                            if 'URL' in moto_row:
...
def guardar_test():
    ...
    if request.method == "POST":
        ...
        # 🔍 DEBUGGING: Verificar campos específicos
        current_app.logger.info("🔍 CAMPOS ESPECÍFICOS RECIBIDOS:")
        for field in ['cilindrada_min', 'cilindrada_max', 'potencia_min', 'potencia_max', 'torque_min', 'torque_max']:
            if field in form_data:
                current_app.logger.info(f"  ✅ {field}: {form_data[field]}")
            else:
                current_app.logger.warning(f"  ❌ {field}: NO RECIBIDO")
        ...
        for field in qualitative_fields:
            if field in form_data and form_data[field] != 'undefined':
                processed_data[field] = form_data[field]
                current_app.logger.info(f"Campo cualitativo: {field} = {processed_data[field]}")
            elif field == 'uso_previsto':
                # Si no hay uso_previsto válido, usar valor por defecto
                processed_data[field] = 'Mixto'
                current_app.logger.info(f"Campo cualitativo: {field} = {processed_data[field]} (valor por defecto)")
        ...
        preferences = {
            ...
            'uso_previsto': processed_data.get('uso_previsto', ''),
        ...
        preferences = {
            ...
            'uso_previsto': processed_data.get('uso_previsto', 'Mixto'),  # ✅ Usar el valor real del test
            'presupuesto_min': processed_data.get('presupuesto_min', 0),
            'presupuesto_max': processed_data.get('presupuesto_max', 100000),
            'cilindrada_min': processed_data.get('cilindrada_min', 0),
            'cilindrada_max': processed_data.get('cilindrada_max', 2000),
